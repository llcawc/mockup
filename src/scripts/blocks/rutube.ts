class LiteRutubeEmbed extends HTMLElement {
  ...
  uriParameters() {
    const parameters: string | unknown = <unknown>(
      Object.fromEntries(Object.entries(this.videoMetadata).filter(([, value]) => !!value))
    )
    return new URLSearchParams(<string>parameters).toString()
  }
}
